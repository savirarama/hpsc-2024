...
int main() {
  ...
  for (int n=0; n<nt; n++) {
    ...
    for (int i=0; i<nx; i++) {
      ...
      u[0][i] = 0;        // u at the top boundary for all x
      u[ny - 1][i] = 1;   // u at the bottom boundary for all x (set to 1 as per Python code)
      v[0][i] = 0;        // v at the top boundary for all x
    v[ny - 1][i] = 0;
    }
    if (n % 10 == 0) {
      for (int j=0; j<ny; j++)
